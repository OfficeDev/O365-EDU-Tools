...
Function Format-ResultsAndExport($graphscopes, $logFilePath) {
    ...
    if($users){
        ...
        Write-Output "no users are found and exit script"
    }
}
...
function Update-SDSUserAttributes
{
	...
	if ($choice -ieq "y" -or $choice -ieq "yes")
	{
		...
		Foreach ($user in $userList) 
		{
			...
			Write-Output "[$(get-date -Format G)] [$index/$userCount] Updating attribute [$($user.userAnchorId)] for user `"$($user.userDisplayName)`" " | Out-File $logFilePath -Append 
            ...
            $index++
            Write-Progress -Activity "Updating SDS user anchor ids" -Status "Progress ->" -PercentComplete ($index/$userlist.count*100)
		}
	}
}
