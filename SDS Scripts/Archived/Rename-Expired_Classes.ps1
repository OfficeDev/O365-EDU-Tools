<#
-----------------------------------------------------------------------
 <copyright file="Rename-Expired_Classes.ps1" company="Microsoft">
 © Microsoft. All rights reserved.
 </copyright>
-----------------------------------------------------------------------
.Synopsis
    Provides ability to cleanup expired sections

.Description
    The script will help in renaming classes setup by School Data Sync. The Displayname and mailNickname
    will be updated, along with 2 extension properties - AnchorId & Status.

.Parameter SectionsToRetireFilePath
    Path to file containing the list of sections to retire.
...
.Example
    .\Rename-Expired_Classes.ps1 -SectionsToRetireFilePath "C:\SectionUsageReport.csv"

#>

Param (
    [Parameter(Mandatory=$true)]
    [string] $SectionsToRetireFilePath,
...
function Generate-UpdateBatchRequest($sections)
{
    ...
    for ($index = 0; $index -lt $sections.Count; $index++) 
    {
        ...
        $anchorId = [string]::Format("Section_{0}_{1}", $SDSExpiredPrefix, $($section.extension_fe2174665583431c953114ff7268b7b3_Education_SyncSource_SectionId))
        $sectionId = [string]::Format("{0}_{1}", $SDSExpiredPrefix, $($section.extension_fe2174665583431c953114ff7268b7b3_Education_SyncSource_SectionId))
        $displayName = [string]::Format("{0}_{1}", $SDSExpiredPrefix, $($section.displayName))
        $mailNickName = [string]::Format("{0}_{1}", $SDSExpiredPrefix, $($section.mailNickName))
...
function Parse-UpdateResponse ($requests, $response) {
    ...
    foreach ($response in $responsesObject.responses) 
    {
        ...
        if ($displayName.IndexOf("$SDSExpiredPrefix ") -gt -1)
        {
            $oldDisplayName = $displayName.Substring($displayName.IndexOf("$SDSExpiredPrefix ") + "$SDSExpiredPrefix ".Length)
        }
...
function Get-SectionsFromFile()
{
    ...
    for ($index = 0; $index -lt $sectionsFromFile.Count; $index = $index + $getByIdsSize) 
    {
        ...
        do
        {
            ...
            | Where-Object {$_.extension_fe2174665583431c953114ff7268b7b3_Education_Status -ne "Expired"} `
            | Select-Object @{Name="objectId";Expression={$_.id}}, displayName, mailNickName, extension_fe2174665583431c953114ff7268b7b3_Education_SyncSource_SectionId, extension_fe2174665583431c953114ff7268b7b3_Education_Status
...
# main
Retire-Sections $SectionsToRetireFilePath $OutFolder $AzureEnvironment
